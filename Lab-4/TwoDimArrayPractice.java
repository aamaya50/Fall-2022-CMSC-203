...
*
* Description: Creates a bar chart from a two dimensional array with different buttons to perform different functions 
* Due: 10/18/2022
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
	   ...
	   for (int i = 0; i < intArray[row].length; i++)	//for loop that iterates through each element in the row
	   {
		   intArray[row][i] = value;	//sets the value to each element in the row
		   animate( row, i);
	   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   ...
	   int minimum = intArray[0][column];	//variable to hold value to compare to other values to find lowest
	   
	   for (int row = 0; row < intArray.length; row++)	//for loop that iterates through each row
	   {
		   if (intArray[row][column] < minimum)	//compares the value at the row and column the minimum variable
		   {
			   minimum = intArray[row][column];	//if it is lower than the minimum variable equals a new lowest value
		   }
		   animate( row, column, minimum);
	   }
      return minimum;
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   ...
	   int num = 0;	//variable that stores the number of times that a value is found
	   
	   for (int row = 0; row < intArray.length; row++)	//nested for loops that goes through each element in each row and column
	   {
		   
		   for (int column = 0; column < intArray[row].length; column++)
		   {
			  if (intArray[row][column] == value)	//if the value if found than the num variable is incremented by one
			  {
				  num += 1;
				  animate ( row, column, num);
			  }
